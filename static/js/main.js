...
var N=5
const battle_id=Array.from(Array(N)).map(()=>S[Math.floor(Math.random()*S.length)]).join('')
...
for(var c of cookiesArray){ //一つ一つ取り出して
    ...
    if( cArray[0] == ' uid'){ // 取り出したいkeyと合致したら
        user_id=cArray[1]
        break
    }
}
...
//set grid
var board_grid = document.getElementById('board_grid')
for (i = 0; i < length; i++) {
...
function End() {
    ...
    board_info = {
        ...
        'method': 'add',
        'hand': hands_log.join('-'),
        'result': user_stone > cpu_stone ? 'win' : user_stone < cpu_stone ? 'lose' : 'tie',
        'user_stone': user_stone,
        'cpu_stone': cpu_stone,
        'board': Board2Str(board),
        'user_color': stone_color[me],
        'battle_id':battle_id,
        'user_id':user_id,
